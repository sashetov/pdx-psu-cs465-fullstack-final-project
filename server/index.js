...
app.get("/join", (req, res) => {
  ...
  if(params.indexOf("playerName") === -1){
    res.status(400).send({ status: "error", message: "missing player name parameter"}); // Bad request
  }
  if(params.indexOf("socket_id") === -1){
    res.status(400).send({ status: "error", message: "missing socket id parameter"}); // Bad request
  }
  ...
  if(gameId && games[gameId].winner == null){
  ...
  else { // create new game since old one is finished or never started
    players[socket_id].playerName = playerName; //set name
    for(let i =0; i < games.length; i+=1){
      let game = games[i];
      if( game["player2"] === null ) { // add second player to started game
    ...
    if(gameId === null) { // initialize new game
      games.push({
        "player1": socket_id,
        "player2": null,
...
let checkBoardForWinner = (gameId) => {
  ...
  */
  ...
  for(let i =0; i < symbols.length; i+=1){
    let s = symbols[i];
    if ( (state[0] === s && state[1] === s && state[2] === s) || //check rows
...
io.sockets.on("connection", socket => {
  ...
  socket.on("move", data => {
    ...
    data.socket_id = id;
    if( players[id].gameId === null){ // houston we have a problem
      ...
      if(game["winner"] === -1){ // error: game not fully initiallized yet
        socket.emit("move_done", {
          "status": "error",
      ...
      else if(game["state"][moveId] !== ""){ // error: slot moved into already
        socket.emit("move_done", {
          "status": "error",
        ...
        if(currPlayer !== game["nextPlayer"]) { // don't play out of turn...
          socket.emit("move_done", {
            "status": "error",
          ...
          if(winner === 0 || winner === 1 || winner === 2) { // game is over, can delete game
            delete games[gameId];
          }
        }
      }
